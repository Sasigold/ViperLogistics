-- 0174: פילוח הכנסות מנהל מערכת, עמלה לקיסר וחלק שיא עיצובים
--
-- 1. פילוח הכנסות (income.mix):
--    - הובלות שיא עיצובים (100%)
--    - לוגיסטיקה לפי לקוח: ארקו, שיא עיצובים, קיסר (בניכוי 10% עמלה מאירועים מעל 2,000 ש"ח)
--    - ריהוט חדש (20% להכנסות)
--    - ריהוט ישן (70% להכנסות)
-- 2. עמלה לקיסר (finance.keisar_commission):
--    - 10% מאירועים של קיסר שסך כל המשימות של האירוע עולה על 2,000 ש"ח
-- 3. הכנסות לשיא עיצובים (finance.client_share):
--    - פילוח בין 80% מריהוט חדש ל-30% מריהוט ישן
    ...
    -- ── הכנסות לשיא עיצובים: 80% מריהוט חדש ו-30% מריהוט ישן עם פילוח ────
      ...
      select jsonb_build_object(
        'total', round(coalesce(sum(case 
                    ...
                    else ei.amount * (100 - ei.viper_share_pct) / 100
                  end), 0), 2),
          ...
          (select coalesce(jsonb_agg(row_to_json(y)), '[]') from (
             select c.name, c.color,
                    round(sum(case 
                      ...
                      else ei2.amount * (100 - ei2.viper_share_pct) / 100
                    end), 2) as total
               from event_income ei2
               join app.live_events e2 on e2.id = ei2.event_id and e2.deleted_at is null
