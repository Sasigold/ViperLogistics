-- 0062: משימה שנמחקה יורדת מלוח העבודה
--
-- הדיווח: "רושם נמחק בהצלחה אבל בפועל ממשיך להציג". שני חלקי המשפט נכונים.
-- soft_delete באמת רץ, deleted_at באמת נכתב — ו-work_board_view ממשיך
-- להחזיר את השורה.
--
-- למה: ל-view אין שום `where`, והוא נשען על tasks_select (0013) שיסנן.
-- הפוליסה אכן מסננת — אבל הזרוע הראשונה שלה היא `app.is_admin()`, שנכונה
-- ללא תנאי. כלומר deleted_at is null נבדק לכל אחד *חוץ* ממנהל, ומנהל הוא
-- בדיוק מי שמוחק. אותו הבדל גם הסתיר את הבאג: אצל עובד המחיקה תמיד עבדה.
--
-- התיקון הוא ב-view ולא בפוליסה. סל המיחזור (0012) חייב לראות שורות מחוקות,
-- ולכן `is_admin` שקוראת אותן הוא נכון; מה שאינו נכון הוא שהלוח — שאין לו
-- מסך שחזור ואין לו עמודה שמסמנת מחיקה — מציג אותן כאילו הן עבודה שתקרה.
--
-- זו אותה שורה שכבר עומדת בכל שאר קוראי המשימות (EventsPage מסנן במפורש,
-- app.planned_shifts_many מסנן, shift_task_breakdown מסנן). ה-view הוא
-- היחיד שלא, ומכאן ההפרש.
