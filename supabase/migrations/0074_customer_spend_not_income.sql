-- 0074: ללקוח אין הכנסות — יש לו הוצאה
--
-- הלקוח במערכת הזאת הוא חברת השכרת ציוד שקונה מאיתנו שירותי הקמה ופירוק.
-- הוא משלם, הוא לא מקבל. ולמרות זאת הדשבורד שלו הציג "הכנסות בטווח",
-- "הכנסות מריהוט", "הכנסות מלוגיסטיקה" ו"הכנסות ללקוח", והתפריט שלו הציג
-- "דוחות" — ובתוכם קטלוג שלם של מדדי עלות ורווחיות פנימיים.
--
-- אף אחד מהם לא הוענק לו במפורש. שלושתם נגזרו:
--
--   dashboard.view  →  reports.view          (0058 §1)
--   pricing.view    →  pricing.revenue       (0017)
--   pricing.revenue →  finance.income_view   (0068)
--
-- ‏0066 §7א העניקה ל-`customer_manager` את `dashboard.view` ואת `pricing.view`
-- — שתיהן נחוצות לו: הדשבורד התפעולי שלו, והרשות לקרוא את התמחור של המשימות
-- שלו (`tp_select`, 0017, דורש `pricing.view` בזרוע הלקוח). מה שלא נבדק הוא
-- מה עוד נגזר מהן. ‏0058 אף כתבה במפורש ש-`reports.view` אינו יכול לדלוף
-- מפני ש-`dashboard.view` הוא `default_allowed = false` — טענה שהייתה נכונה
-- כשנכתבה, ו-0066, שהגיעה אחריה, ביטלה אותה בלי לשים לב.
--
-- שכבת ה-kind היא המקום הנכון לסגור את זה ולא התפקיד: היא חלה על כל תפקיד
-- לקוח שייכתב בעתיד, כולל כאלה שעוד לא קיימים. התפקידים נסגרים כאן *בנוסף*,
-- מפני ששכבת התפקיד גוברת על שכבת ה-kind — ולכן שורת allow קיימת (וכזו יש:
-- `customer_manager` מחזיק `attendance.view_own` מאז 0011) הייתה מנצחת את
-- החסימה שלמטה.
--
-- ומה כן: `finance.customer_spend` — כמה הוא הוציא. זה המספר היחיד בכספים
-- שיש לו משמעות מהצד שלו, והוא נגזר מאותו `task_pricing.price` שהיה מוצג לו
-- קודם תחת הכותרת ההפוכה. אותו נתון, השם הנכון, ובלי שאר הפילוחים.

-- ===== 1. מה שנסגר בשכבת סוג המשתמש ========================================
--
-- ‏`pricing.view` נשאר פתוח: בלעדיו הלקוח אינו יכול לקרוא את התמחור של
-- המשימות שלו, וזה בדיוק הנתון שההוצאה נבנית ממנו. מה שנסגר הוא מה שנגזר
-- ממנו כלפי מעלה.
...
select 'customer_user'::user_kind, k, false
  from unnest(array[
    ...
    'pricing.revenue',
    'finance.income_view',
    'finance.receipts_view',
    -- דוחות: כל הקטלוג נגזר מ-reports.view, וגם מה שמסרב להחזיר נתונים
    -- מציג ללקוח את שמות המדדים הפנימיים בבורר
    'reports.view',
    -- נוכחות: ללקוח אין שעון ואין משמרות, לא למנהל ולא לעובדים שלו
    'attendance.view_own',
    'attendance.view_own_pay',
    'attendance.view_schedule',
    'attendance.view_all',
    'attendance.clock',
    'attendance.submit_entry',
    'portal.attendance'
  ]) as k
on conflict (user_kind, permission_key) do update set allowed = false;

-- ===== 2. ...ובשכבת התפקיד, שגוברת עליה ====================================
...
update role_permissions rp
 ...
 where r.id = rp.role_id
   ...
   and rp.permission_key in (
     'pricing.revenue', 'finance.income_view', 'finance.receipts_view',
     'reports.view',
     'attendance.view_own', 'attendance.view_own_pay', 'attendance.view_schedule',
     'attendance.view_all', 'attendance.clock', 'attendance.submit_entry',
     'portal.attendance');

-- ודחייה מפורשת גם למי שאין לו שורה בכלל, כדי שהתשובה לא תישען על ירושה
-- שתשתנה בעתיד.
insert into role_permissions (role_id, permission_key, allowed)
select r.id, k, false
  from permission_roles r
 cross join unnest(array[
   'pricing.revenue', 'finance.income_view', 'finance.receipts_view', 'reports.view',
   'attendance.view_own', 'attendance.view_own_pay', 'attendance.view_schedule',
   'attendance.view_all', 'attendance.clock', 'attendance.submit_entry',
   'portal.attendance'
 ]) as k
...
-- ===== 3. המפתח היחיד שנפתח: כמה הוצאתי ====================================
--
-- בלי `implied_by`. הפיתוי היה לגזור אותו מ-`pricing.view`, וזו בדיוק הטעות
-- שהמיגרציה הזאת מתקנת: `pricing.view` מוחזק גם בידי אנשי משרד, והכרטיס
-- "ההוצאה שלי" היה מופיע אצלם ומציג את סך התמחור של כל הלקוחות תחת כותרת
-- שאומרת "שלי". מפתח שמשמעותו תלויה במי שואל צריך להינתן, לא להיגזר.

select app.register_permission('finance.customer_spend', 'finance',
       'צפייה בהוצאה על האירועים שלי',
       p_description => 'סך מה שחויב על המשימות של הלקוח — הצד שלו של אותו תמחור',
       p_category => 'access',
       p_applies_to => array['customer_user']::user_kind[]);

insert into role_permissions (role_id, permission_key, allowed)
select r.id, 'finance.customer_spend', true
  from permission_roles r where r.key = 'customer_manager'
on conflict (role_id, permission_key) do update set allowed = true;

-- ===== 4. הסקשן ============================================================
--
-- `security invoker` כמו קודמו, ולכן `tp_select` (0017) הוא שתוחם את השורות
-- ללקוח הקורא — הפונקציה לא מסננת לפי `customer_id` בעצמה. איש משרד שיקרא
-- את הסקשן הזה לא יגיע לכאן בכלל: אין לו את המפתח.
--
-- הגוף מועתק מ-0069 §3 כלשונו, עם ענף אחד נוסף. זו המוסכמה בקובץ הזה —
-- ‏0041 → 0069 עשו בדיוק אותו דבר — ולכן מי שמוסיף סקשן בעתיד ימשיך מכאן.
