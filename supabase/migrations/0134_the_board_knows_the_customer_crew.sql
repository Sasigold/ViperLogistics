-- 0134: הלו״ז מכיר את סגל הלקוח
--
-- ‏0133 נתן ללקוח שמבצע בעצמו רשומת סגל ו-RPC לשבץ ממנה. כאן היא נכנסת ללו״ז,
-- שהוא המסך היחיד שהלקוח הזה עובד בו: `customer_worker_list` לצד
-- `contractor_worker_list`, ונסיגה שלישית לשורת ראש הצוות.
--
-- **סדר הנסיגה הוא סדר ההכרעה:** שיבוץ פנימי, אחר כך ראש הצוות של הקבלן,
-- ואחרון סגל הלקוח. במשימה שסומנה "ארקו" אין שיבוץ פנימי (0135 מנקה אותו),
-- ולכן בפועל היא רואה את שלה — והסדר מבטיח שמשימה שחזרה לוייפר לא תמשיך
-- להציג ראש צוות של מי שכבר אינו מבצע אותה.
--
-- הגוף זהה ל-0128, בתוספת שני ה-lateral, הנסיגה, והעמודה החדשה בסוף — עמודה
-- חדשה נוספת תמיד בסוף, כי `create or replace view` אינו מרשה להכניס באמצע.
