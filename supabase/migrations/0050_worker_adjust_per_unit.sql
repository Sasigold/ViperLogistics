-- 0050: תוספת עובדים לפי כמות — "אין חניה ⇒ עובד לכל משאית"
--
-- תוספת העובדים במודל worker_hours (0017) ידעה עד היום דבר אחד: להוסיף מספר
-- קבוע כשתנאי מתקיים. זה נכון ל"נדרש ראש צוות ⇒ +1", ולא נכון לחניה: אירוע
-- בלי חניה עם משאית אחת ואירוע בלי חניה עם ארבע אינם אותו מאמץ, כי כל
-- משאית מתפרקת רחוק ומישהו צריך ללכת איתה. מי שרצה את זה נאלץ עד היום
-- לפתוח שורת תוספת נפרדת לכל כמות משאיות עם תנאי eq, וזה רשימה שנשברת
-- בכמות החמישית.
--
-- לכן לתוספת יש עכשיו kind:
--
--   fixed    (ברירת מחדל, וההתנהגות הקיימת) — add עובדים, כמספר.
--   per_unit — add עובדים לכל יחידה של input. "אין חניה ⇒ עובד לכל משאית"
--              הוא input=truck_count, add=1, when=no_parking is_true.
--
-- rounding שייך ל-per_unit בלבד, כי חצי עובד אינו אדם: 0.5 לכל משאית על
-- שלוש משאיות הוא 1.5, וההגדרה היא זו שאומרת אם גובים 1 או 2.
--
-- קונפיגים קיימים אינם משתנים: תוספת בלי kind נקראת כ-fixed, בדיוק כפי
-- שנקראה קודם. גם תבניות ברירת המחדל (app.default_pricing_config) נשארות
-- כפי שהן — מי שרוצה את הספירה לפי משאיות מדליק אותה במחשבון של הלקוח.
--
-- הפונקציה נכתבת כאן במלואה כי אין דרך אחרת להחליף גוף של פונקציה ב-SQL.
-- שאר הגוף זהה ל-0017; מה שהשתנה הוא לולאת תוספות העובדים בלבד.
