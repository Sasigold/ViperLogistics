...
# 42 מקימה מחסן, לקוח, אירוע, ארבעה עובדים וחמש משימות משלה ב-current_date + 590,
...
OUT42=$($PSQL -d vl -f "$HERE/42_the_shift_ends_where_the_last_task_ends.sql" 2>&1 | grep -v '^[0-9a-f-]\{36\}$' | grep -v '^$')
echo "$OUT42"
OUT="$OUT
$OUT42"
