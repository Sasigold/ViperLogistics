...
select t_expect_fail('יותר מדי סקשנים בבקשה אחת נדחה',
  $$select dashboard_sections(
      ...
      -- הצי), ל-51 ב-0143 (`customer.monthly`) ול-60 ב-0174 (פירוט ההכנסה
      -- והעמלות של מנהל המערכת). הבדיקה רודפת אחריה במכוון: תקרה שעלתה בלי
      -- שאיש שם לב היא בדיוק מה שהיא נועדה לתפוס.
      (select array_agg('k' || g) from generate_series(1,61) g),
