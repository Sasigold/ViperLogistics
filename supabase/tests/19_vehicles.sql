...
/* זו ההחלטה של 0088 §1, והבדיקה קיימת כדי שלא תבוטל בשקט: `fleet.view` הוא
...
/* 0088 §3: הרכב הוא מקור האמת, והטריגר דוחף ממנו את הרישוי אל המשאית. */
...
/* 0088 §5: הכתיבה הישירה נחסמת, אחרת העמודה הנגזרת הופכת למקור אמת שני. */
...
/* והדלת האחורית: `app.audit()` הגנרית הייתה כותבת את ה-PIN ל-audit_log,
   שנקרא במפתח אחר לגמרי. 0088 §6 מחליף אותה בטריגר שמסתיר את שני השדות. */
select set_config('request.jwt.claim.sub', '00000000-0000-0000-0000-0000000019a1', false);
