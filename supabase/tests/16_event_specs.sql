...
-- ...וניהול המפרט נשאר של הצוות: events.edit פתוח למשתמש לקוח בברירת המחדל של
-- סוג המשתמש, ולכן בלי שורת ה-kind של 0102 §3 הוא היה יורש את מפתח הניהול.
select t_eq('אך מפתח הניהול סגור בפניו',
  app.has('events.specs_manage'), false);

select t_expect_fail('ולכן גם גרסת קישור אינה נכנסת ממנו',
  $$insert into event_specs (event_id, source, url)
    values ('30000000-0000-0000-0000-000000000016', 'link', 'https://x.test/client')$$);

select t_eq('והשער על ה-Storage סגור לו לכתיבה',
  app.may_touch_event_spec(
    '30000000-0000-0000-0000-000000000016/aaaaaaaa-0000-0000-0000-000000000001.pdf', true), false);
