...
--
-- החבילה מקימה לקוח, אירוע וחמישה פרופילים משלה ואינה נשענת על אף חבילה קודמת —
...
insert into auth.users (id, email) values
  ...
  ('00000000-0000-0000-0000-0000000016a5', 'specs-client@vl.test');
...
-- משתמש הלקוח נכנס בנפרד: הוא היחיד עם customer_id, והוא הקהל של 0102 —
-- אין לו שום שורת הענקה, וכל מה שיש לו הוא האירוע של הלקוח שלו.
insert into profiles (id, user_id, user_kind, is_admin, full_name, customer_id) values
  ('20000000-0000-0000-0000-0000000016a5', '00000000-0000-0000-0000-0000000016a5',
   'customer_user', false, 'איש קשר אצל הלקוח', '10000000-0000-0000-0000-000000000016');
...
-- ===== 10. משתמש פורטל רואה את המפרט (0102) ================================
-- עד 0102 פוליסת ה-select דרשה `user_kind = 'staff'`, ולכן איש הקשר אצל הלקוח
-- קיבל רשימה ריקה על האירוע של עצמו. עכשיו הצפייה נגזרת מהאירוע בלבד — ואין
-- לו שום שורת הענקה, כך שמה שנבדק כאן הוא ברירת המחדל של המרשם.

select set_config('request.jwt.claim.sub', '00000000-0000-0000-0000-0000000016a5', false);

select t_eq('משתמש הלקוח מחזיק את מפתח הצפייה בלי הענקה',
  app.has('events.specs_view'), true);

select t_eq('והוא רואה את הגרסאות החיות של האירוע שלו',
  (select count(*)::int from event_specs
    where event_id = '30000000-0000-0000-0000-000000000016'), 3);
...
-- ...וניהול המפרט — כלומר ההסרה — נשאר של הצוות: events.edit פתוח למשתמש לקוח
-- בברירת המחדל של סוג המשתמש, ולכן בלי שורת ה-kind של 0102 §3 הוא היה יורש
-- את מפתח הניהול.
select t_eq('אך מפתח הניהול סגור בפניו',
  app.has('events.specs_manage'), false);

-- ===== 10א. ומהלקוח מגיע המפרט (0113) ======================================
-- ‏0102 השאירה גם את ההעלאה אצל המשרד. ‏0113 מפרידה בין השתיים: ההעלאה מקבלת
-- מפתח משלה, שברירת המחדל של סוג המשתמש פותחת ללקוח וסוגרת לקבלן, וההסרה
-- נשארת ב-`events.specs_manage`. הבדיקה כאן היא שהפיצול באמת נפרד — הלקוח
-- מעלה בלי שום שורת הענקה, ועדיין אינו מסיר.

select t_eq('משתמש הלקוח מחזיק את מפתח ההעלאה בלי הענקה',
  app.has('events.specs_upload'), true);

select t_expect_ok('ולכן גרסת קישור נכנסת ממנו',
  $$insert into event_specs (event_id, source, url)
    values ('30000000-0000-0000-0000-000000000016', 'link', 'https://x.test/client')$$);

select t_eq('והשער על ה-Storage פתוח לו גם לכתיבה',
  app.may_touch_event_spec(
    '30000000-0000-0000-0000-000000000016/aaaaaaaa-0000-0000-0000-000000000001.pdf', true), true);

-- אבל הוא אינו מוריד מההיסטוריה: `remove_event_spec` שואל את מפתח הניהול,
-- וזה בדיוק ההבדל שהפיצול קיים בשבילו. גרסה שגויה מתוקנת בגרסה הבאה.
select t_expect_fail('ולהסיר גרסה הוא עדיין אינו יכול',
  $$select remove_event_spec(
      (select id from event_specs
        where event_id = '30000000-0000-0000-0000-000000000016' order by version desc limit 1))$$);

reset role;
select set_config('request.jwt.claim.sub', '', false);

-- הקבלן, לעומתו, נשאר בצפייה בלבד — הוא מבצע את מה שכתוב במפרט ואינו צד
-- לתוכן שלו. אין לו פרופיל בחבילה הזו, ולכן זה נבדק על שורת ברירת המחדל.
select t_eq('ולקבלן ברירת המחדל סוגרת את ההעלאה',
  (select allowed from kind_permission_defaults
    where user_kind = 'contractor_user' and permission_key = 'events.specs_upload'), false);
