...
- **מאיזה מחסן** — `warehouses` היא טבלה, ו-`tasks.warehouse_id` נוקב באחד מהם.
  המשימה אינה חייבת לנקוב: כשהיא לא, ההחתמה נמדדת מול המחסן ה*קרוב ביותר*
  לעובד. זו לא הקלה — הוא עדיין חייב להיות בתוך רדיוס של מחסן אמיתי, ורק
  השאלה "איזה מהם" נענית לפי המיקום במקום לפי התיוג. רדיוס פר-מחסן גובר על
  הגלובלי, ודריסה אישית של העובד גוברת על שניהם.
...
```
supabase/migrations/   # 0001..0022 — סכמה, זריעה, RLS, RPCs, הרשאות, תמחור, נוכחות
