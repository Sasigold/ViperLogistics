...
מסך `/warehouse`: לכל אירוע של ארקו שתי עמודות — **הכנה** (ביום ההקמה) ו**החזרה**
(ביום הפירוק), ובכל אחת: בשעה, מס׳ תעודה, שם לקוח, זמן, הערות, אישור סופי,
אירוע מוכן ובדיקה. לחיצה על כותרת העמודה מזיזה אותה ליום אחר (ומחזירה לנגזר).
הלו״ז אינו נוגע במשימות, במחירים או ב-webhook לארקו — זו טבלה משלו.

| | |
|---|---|
| השער | המתג "לו״ז מחסן" בכרטיס הלקוח (`customers.warehouse_schedule_enabled`). המיגרציה הדליקה אותו ללקוח ששמו מכיל "ארקו" |
| יוזר מחסן | בעובדים: משתמש מסוג **לקוח**, הלקוח **ארקו**, ובלשונית ההרשאות — התפקיד **"מחסן"**. הוא רואה ומעדכן את לו״ז המחסן, ותו לא |
| מנהל אצל הלקוח | רואה ועורך את אותו לו״ז בדיוק (`warehouse.view` + `warehouse.edit`) |
| צופה אצל הלקוח | רואה בלבד |
| משרד | מנהל תפעול ורכז שיבוצים — צפייה ועריכה; צופה — צפייה. כל השאר דרך מטריצת ההרשאות, מודול "לו״ז מחסן" |
