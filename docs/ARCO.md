...
כל משלוח מארקו מוציא התראה **לכל מנהלי המערכת**, בסוג אחד — **"הזמנה
מארקו"** (`arco_order_received`, קבוצת "אירועים" במטריצת ההתראות):

| מה קרה | כותרת ההתראה | הגוף |
|---|---|---|
| נפתח אירוע חדש | אירוע חדש מארקו | הלקוח · מספר הזמנה · הלקוח הסופי · תאריך · מיקום |
| עדכון ששינה משהו | ארקו עדכנה הזמנה | מה השתנה, "שדה: חדש (היה ישן)" — לפי יומן הפעילות |
| מפרט חדש | מפרט חדש מארקו | הלקוח · מספר הזמנה · תאריך |
| המשלוח נכשל | הזמנה מארקו לא נקלטה | מספר ההזמנה והסיבה |

**משלוח חוזר שלא שינה דבר אינו מוציא התראה** — ‏Make שולח את אותה הזמנה
פעמים רבות. משלוח שסומן `ignored` (חיבור כבוי, מפרט זהה, מפרט שהקדים את
ההזמנה) שותק גם הוא. לחיצה על ההתראה פותחת את האירוע. בתוך המערכת ההתראה
מאולצת; פוש ומייל לפי המטריצה, ואפשר לצמצם אותה ללקוחות מסוימים דרך התחולה.
