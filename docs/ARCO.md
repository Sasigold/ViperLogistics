...
| `order_date` | תאריך האירוע. הצינור מעביר `MM/DD/YYYY HH:mm` — אמריקאי, לא ישראלי (0186); ‏ISO עם אזור מומר לשעון ישראל |
...
**והתאריך אמריקאי.** ‏`12/07/2026` הוא **7 בדצמבר**, לא 12 ביולי. זו הכרעה
שנלמדה מהשדה: הזמנה אמיתית נשאה `09/28/2026`, ואין חודש 28. מה שאינו יכול
להיות חודש נקרא כיום; כל השאר נקרא `MM/DD`. טעות כאן אינה נראית לעין —
שתי הקריאות "מצליחות", והמשאית פשוט מגיעה בחודש הלא נכון.

**ושדה ריק אינו אפס.** ארקו שולחת `""` בשדה מספרי שלא מולא (כמות משאיות,
עובדים, שעות). אצלנו זה `null` — "לא נאמר" — ולכן משלוח כזה אינו מאפס את
מה שהרכז קבע.
