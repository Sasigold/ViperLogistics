...
‏`users/{uid}/record` מחזיקה את רשומות השעון. הועברו שמונה עובדים בלבד,
לפי בקשה: שמעון באך, אביתר עמדי, שלומי אברמס, איציק אספיס, אבי מיארה,
מחמוד אבו כמאל (במקור "מוחי אבו קמאל"), אברהיים מנסה, וואניס מנסה. שמונתם
כבר קיימים כפרופילים; ההתאמה נעשתה לפי שם ואומתה מול הטלפון.

```bash
node fetch-records.mjs                                   # → dump/records.json
node build-attendance.mjs ./dump profiles.json ./out-att
```

‏`profiles.json` ממפה מזהה משתמש ב-Firestore ל-`profiles.id`.
‏`dump/taken.json` (רשות) הוא רשימת (profile, date, seq) שכבר תפוסים
בטבלה, כדי שהמספור ידלג עליהם.

| Firestore | עמודה |
| --- | --- |
| `start` | `clock_in_at`, ומ-`work_date` נגזר התאריך בשעון ישראל |
| `end` | `clock_out_at` |
| `location` / `locationSiom` | `clock_in_lat/lng`, `clock_out_lat/lng` |
| `bonos` | `attendance_entry_bonus` (רק כשחיובי — יש אילוץ) |
| `sacar` | `worker_pay_settings.hourly_rate` — האחרון שאינו אפס |
| `daStEdit` / `daEnEdit` | דגל `edited` |
| `mishmeretRef` חסר | דגל `no_shift` |

מה שנטען: 2,035 רשומות, 886 בונוסים, 8 שורות תעריף, וארכיון מלא תחת
`collection='record'`. כל שורה נושאת את הדגל `legacy_import`.

‏`ragil`, `no125` ו-`no150` אינם נטענים: מנוע השעות הנוספות כאן גוזר אותם
מ-`actual_hours` ומהגדרות השכר. הם נשמרים בארכיון.

‏**שלוש רשומות נטענו מסומנות כמחוקות** — שתי משמרות שנפתחו ולא נסגרו לפני
חודשים ואחת באורך אפס. ‏`attendance_one_open` מתיר משמרת פתוחה אחת לעובד
ו-`attendance_order` דורש יציאה שאחרי הכניסה; מעבר לאילוץ, משמרת נטושה
הייתה מציגה עובד כמוחתם מאפריל.

‏**‏142 רשומות ארוכות מ-16 שעות, מהן 26 ארוכות מ-24** — משמרות שנשכחו
פתוחות במערכת הישנה (הארוכה ביותר 138 שעות). הן נטענו כפי שהן, כי כך הן
במקור; מי שמכיר את המשמרות יחליט אם לתקן.
